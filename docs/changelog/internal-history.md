# Archived Legacy Spec

This file is an archived historical document.

- Canonical docs entry: `docs/index.md`
- Pointer stub: `docs/claude.md`
...
## 19. Development Testing Changes (v1.2.0-dev)

### 19.1 Frontend Fixes

1. **QueryClientProvider Fix**
   - **Issue:** All admin pages except Dashboard showed blank screen with `No QueryClient set` error.
   - **File:** `admin/js/src/App.jsx`
   - **Fix:** Wrapped entire application in `QueryClientProvider` with sensible defaults:
     ```jsx
...
### 19.2 REST API Architecture Changes

1. **REST Controller Switch**
   - **Issue:** `RestApi.php` was missing `/logs` and `/settings` endpoints.
   - **File:** `includes/Plugin.php`
   - **Fix:** Switched from `RestApi` to `RestController` + `KBController`:
     ```php
...
2. **Settings Endpoint Added**
   - **File:** `includes/REST/RestController.php`
   - **Endpoints:** `GET /settings`, `POST /settings`
   - **Methods:** `get_settings()`, `update_settings()`

### 19.3 Debug Logging Infrastructure

Comprehensive debug logging added for development testing:

| Component | Logs Added |
|-----------|-----------|
| `Plugin.php` | Plugin init, version, DB upgrade, admin renderer, REST API init, all hook registrations |
| `RestController.php` | Route registration, endpoint calls |

**Log Location:** `wp-content/uploads/vibe-ai-logs/YYYY-MM-DD.log`

**Log Levels:** `debug`, `info`, `warning`, `error`

To enable debug logging, the Logger class writes to daily log files. View logs via:
- Admin UI: AI Entity Index → Logs tab
- REST API: `GET /wp-json/vibe-ai/v1/logs`
- Direct file access: `wp-content/uploads/vibe-ai-logs/`
...
powershell -Command "
  ...
  New-Item -ItemType Directory -Force build_temp/ai-entity-index | Out-Null;
  Copy-Item ai-entity-index.php build_temp/ai-entity-index/;
  Copy-Item -Recurse includes build_temp/ai-entity-index/;
  Copy-Item -Recurse vendor build_temp/ai-entity-index/;
  New-Item -ItemType Directory -Force build_temp/ai-entity-index/admin/js | Out-Null;
  Copy-Item -Recurse admin/js/build build_temp/ai-entity-index/admin/js/;
  cd build_temp; tar -a -c -f ../dist/ai-entity-index.zip ai-entity-index;
"
```

### 19.5 Known Issues

1. **409 Conflict on Pipeline Start** - Dashboard shows conflict error when starting pipeline if system believes pipeline is already running. Check `wp_options` for stale transients.

2. **Lint Warnings** - IDE shows warnings for WordPress functions (`add_action`, `current_user_can`, etc.) because WordPress stubs are not installed. These are not actual errors.
