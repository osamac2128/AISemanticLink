...
function StatusBadge( { status } ) {
	const statusConfig =
		ENTITY_STATUSES.find( ( s ) => s.value === status ) ||
		ENTITY_STATUSES[ 0 ];
		...
		() => [
					...
					if ( isEditing ) {
						return (
							<InlineTextInput
								value={ entity.name }
								onSave={ ( value ) =>
									onInlineEdit( entity.id, 'name', value )
								}
			...
			// Type column (sortable, dropdown filter)
			{
				...
				cell: ( { row } ) => {
					...
					const typeLabel =
						ENTITY_TYPES.find( ( t ) => t.value === entity.type )
							?.label || entity.type;
					...
					if ( isEditing ) {
						return (
							<InlineTypeSelect
								value={ entity.type }
								onSave={ ( value ) =>
									onInlineEdit( entity.id, 'type', value )
								}
								onCancel={ () => onEditingCellChange( null ) }
							/>
						);
		...
		return (
			<div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
				...
				<p className="mt-1 text-sm text-gray-500">
					Start by running the extraction pipeline to discover
