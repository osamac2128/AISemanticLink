...
export function EntityDrawer( { isOpen, onClose, entityId } ) {
	...
	const {
		...
		queryFn: () => apiFetchEntity( entityId ),
	...
	const updateMutation = useMutation( {
		mutationFn: ( { id, data } ) => apiUpdateEntity( id, data ),
		onSuccess: () => {
	...
	const handleAddAlias = useCallback( ( alias ) => {
		setFormData( ( prev ) => {
			...
			if ( prev.aliases.some( ( a ) => a.alias === alias ) ) {
				return prev;
			} // Already exists
			const updated = {
				...prev,
				aliases: [
					...
					{
						alias,
						alias_slug: alias.toLowerCase().replace( /\s+/g, '-' ),
					},
				],
			};
			setHasChanges( true );
