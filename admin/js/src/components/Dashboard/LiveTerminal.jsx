 ...
 * @return {JSX.Element} Log entry element.
 */
function LogEntry( { entry } ) {
	const levelConfig =
		LOG_LEVELS[ entry.level?.toUpperCase() ] || LOG_LEVELS.INFO;
	...
	return (
		<div className="terminal-line flex gap-3 py-0.5 hover:bg-slate-800/50">
			...
			<span className="text-slate-500 flex-shrink-0 select-none">
				[
				{ formatTimestamp(
					entry.timestamp || entry.time || Date.now()
				) }
				]
			</span>
			...
			{ /* Level badge */ }
			<span
				className={ `flex-shrink-0 font-semibold w-14 ${ levelConfig.className }` }
			>
				{ levelConfig.label }
			</span>
 ...
 * @return {JSX.Element} LiveTerminal element.
 */
 ...
 */
LiveTerminal.Compact = function CompactTerminal( {
	logs = [],
	maxEntries = 5,
} ) {
	const displayedLogs = logs.slice( -maxEntries );
